...
class TextClassificationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        text = self.data.iloc[idx]["article_text"]
        # Convert string label to numeric using the label_map
        label = self.data.iloc[idx]["llm_bias_cat"]
...
def main():
    ...
    train_dataloader, val_dataloader = load_datasets(
        "../data/topics_10k_deepseek.csv", label_map, tokenizer)
    ...
    writer = SummaryWriter(log_dir="./runs/exp-distill")
